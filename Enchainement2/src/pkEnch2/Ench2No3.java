...
public class Ench2No3 
{
	...
	{
				String strPrenom;				//Saisie du prénom de l'élève
				String strNom;					//Saisie du nom de l'élève
		...
		final	double D_EXAM 		= 0.2;		//Pondération des examens sur la note total
		final	double D_TRAV 		= 0.1;		//Pondération des examens sur la note total
		
				double dNoteFinale;				//Variable pour calcul note finale
		
		NumberFormat pourcentage = NumberFormat.getPercentInstance();
		
		strPrenom 		= JOptionPane.showInputDialog("Entrez le prénom de l'élève.");
		...
		dNoteFinale 	/= 100;				//Ici je divise par 100 pour que le format pourcentage fonctionne même s'il multiplie par 100
		
		JOptionPane.showMessageDialog(null, strPrenom + " a obtenu " + pourcentage.format(dNoteFinale) + " pour son cours de programmation orientée objet.", "Note Finale", JOptionPane.PLAIN_MESSAGE);
