...
public class Tab2DSupNo1 
{
	...
	public static void main(String[] args) 
	{
		final	int		PASSAGE		= 60;																			//Constante de la note de passage
				...
				int		iEtudiant	= 0;																			//Nombre d'Ètudiant
				int		iReussi		= 0;																			//Nombre d'Ètudiant ayant rÈussi
				int		iEchec		= 0;																			//Nombre d'Ètudiant ayant Èchouer
		...
		for (int iCol = 0 ; iCol < iNote[0].length ; iCol++) 
		{
			...
			if (dMoy >= PASSAGE)
				{
					strMoy	+= "\nLa moyenne des notes d'examens pour l'Ètudiant " + (iCol + 1) + " est de:\t" + deux.format(dMoy) + "%\tR…USSI";
					iReussi++;
				}
			else
				{
					strMoy	+= "\nLa moyenne des notes d'examens pour l'Ètudiant " + (iCol + 1) + " est de:\t" + deux.format(dMoy) + "%\t…CHEC";
					iEchec++;
				}
		...
		strInfo	+= "\n\nLe nombre d'Ètudiants ayant un Èchec:\t\t" + iEchec;
		strInfo	+= "\nLe nombre d'Ètudiants ayant un rÈussis le cours:\t" + iReussi;
		strInfo	+= "\nLe nombre d'Ètudiants:\t\t\t" + iEtudiant;
